fun main() {
    ...
    fun List<String>.prepare(): List<List<Int>> =
        map { row -> row.split("").filterNot { it.isEmpty() }.map { it.toInt() } }

    fun List<List<Int>>.neighbours(rowIndex: Int, index: Int): Set<Pair<Int, Int>> {
    ...
    fun List<List<Int>>.minimums(): Set<Pair<Int, Int>> {
        val s = mutableSetOf<Pair<Int, Int>>()
    ...
    fun List<List<Int>>.basin(pos: Pair<Int, Int>, result: MutableSet<Pair<Int, Int>>) {
